                                    ...
                                    foreach ($datam as $item) {
                                        ...
                                        if (!isset($groups[$key])) {
                                            $groups[$key] = array(
                                                ...
                                                'gr_parts' => $item->quantity,
                                                'gr_rate' => $item->cutting_rate_a,
                                                'gr_amount' => ($item->receive_cut_quantity * $item->quantity * $item->cutting_rate_a)
                                            );
                                        } else {
                                            $groups[$key]['ord_nu'] = $item->cut_rcv_number;
                                            $groups[$key]['gr_sum'] += $item->receive_cut_quantity;
                                            $groups[$key]['gr_parts'] += $item->quantity;
                                            $groups[$key]['gr_rate'] += $item->cutting_rate_a;
                                            $groups[$key]['gr_amount'] += ($item->receive_cut_quantity * $item->quantity * $item->cutting_rate_a);
                                        }
                                    }
                                   ...
                                   <?php }
                                    ...
                                    <tr>
                                        ...
                                        $grand_total_receive_cut_quantity += $d->receive_cut_quantity;
                                         ?></td>
                                         <?php 
                                         $quantity_values = $this->db->select('sum(quantity) as quantity')->get_where('article_parts', array('am_id' => $d->am_id, 'ig_id !=' => 1))->row();
                                         if(count($quantity_values) > 0) {
                                             $quantity = $quantity_values->quantity;
                                         } else {
                                             $quantity = 0;
                                         }
                                         ?>
                                        ...
                                        echo $quantity;
                                        $grand_total_parts += $quantity;
                                        ?></td>
                                        <td class="text-right"><?php
                                        $total_rate += $d->cutting_rate_a;
                                        echo $d->cutting_rate_a; 
                                        $grand_total_rate += $d->cutting_rate_a;
                                        ?></td>
                                        <td class="text-right">
                                            <?php
