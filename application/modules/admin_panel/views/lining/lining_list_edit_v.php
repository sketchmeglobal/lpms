...
<html lang="en">
<head>
    <title>Lining List Edit | <?=WEBSITE_NAME;?></title>
...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <!-- page head start-->
        <div class="page-head">
            <h3 class="m-b-less">Lining List Edit</h3>
            <div class="state-information">
                <ol class="breadcrumb m-b-less bg-less">
                    <li><a href="<?=base_url('admin/dashboard');?>">Home</a></li>
                    <li class="active"> Lining List Edit</li>
                </ol>
            </div>
        </div>
        <!-- page head end-->
        ...
        <div class="wrapper">
                ...
                <div class="col-md-12">
                    <section class="panel">
                        <header class="panel-heading">
                            Edit: <?= $lining_details[0]->employees_name ?>
                        ...
                        <?php #print_r($purchase_order_details); die;?>
                            <form id="form_lining_list_edit" method="post" action="<?=base_url('admin/form-lining-list-edit')?>" class="cmxform form-horizontal tasi-form">
                            ...
                            <div class="col-lg-3">
                                    ...
                                    <option value="">Select Employee</option>
                                            ...
                                            ?> 
                                                <option value="<?= $emp_detail->e_id ?>" short_name="<?=$emp_detail->name?>" e_code="<?=$emp_detail->e_code?>" <?php if($emp_detail->e_id == $lining_details[0]->e_id){?> selected <?php } ?> ><?= $emp_detail->name . ' ['. $sn .']' ?></option>
                                                <?php
                                            }
                                            ?>
                                    </select>
                            ...
                            <div class="col-lg-3">
                                        ...
                                        <input id="lining_start_date_time" name="lining_start_date_time" type="date" placeholder="Start Date Time" class="form-control round-input" value="<?= date('Y-m-d', strtotime($lining_details[0]->lining_entry_date)) ?>" />
                                    </div>
                            <div class="col-lg-2">
                                                    <label for="extra_time" class="control-label text-danger">Extra Time</label>
                                                    <input type="number" id="extra_time" name="extra_time" value="<?= $lining_details[0]->extra_time ?>" class="form-control" />
                                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-3 col-sm-3">
                                        <button class="btn btn-success" type="submit"><i class="fa fa-refresh"> Update Lining</i></button>
                                    </div>
                                    <!--<div class="col-sm-3">
                                        <button id="print_all" type="button" class="btn btn-primary"><i class="fa fa-print"></i> Print</button>
                                    </div>-->
                                </div> 
                                <input type="hidden" id="lining_id" name="lining_id" class="hidden" value="<?= $lining_details[0]->lining_id ?>" />
                            </form>
                        </div>
                    </section>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <section class="panel">
                        <header class="panel-heading">
                            Add Lining details for: <?= $lining_details[0]->employees_name ?>
                            <span class="tools pull-right">
                                <a class="t-collapse fa fa-chevron-down" href="javascript:;"></a>
                            </span>
                        </header>
                        <div class="panel-body">
                            ...
                            <div class="tab-content">
                                ...
                                <div id="cut_issue_challan_list" class="tab-pane fade in active">
                                    <table id="lining_list_details_table" class="table data-table dataTable">
                                        <thead>
                                            <tr>
                                                <th>Lining Type</th>
                                                <th>Order Number</th>
                                ...
                                <div id="cut_issue_challan_add" class="tab-pane fade">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3">
                                            ...
                                            <input type="text" name="lining_id" id="lining_id" class="hidden" value="<?= $lining_details[0]->lining_id ?>" />
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div id="cut_issue_receive_edit" class="tab-pane">
                                    ...
                                    <div class="row">
                                        ...
                                        <input type="text" name="lining_id" id="lining_id" class="hidden" value="<?= $lining_details[0]->lining_id ?>" />
                                        <input type="text" name="lining_detail_id_edit" id="lining_detail_id_edit" class="hidden" value="" />
                                    </form>
                                    </div>
                                </div>
                            </div>
...
$("#am_id").change(function(){
    ...
    $(document).ready(function() {
        $('#lining_list_details_table').DataTable( {
            "processing": true,
            "language": {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('admin/ajax-lining-list-details-table-data')?>",
                "type": "POST",
                "dataType": "json",
                data: {
                    lining_id: function () {
                        return $("#lining_id").val();
                    },
                },
            },
...
//     });
    ...
    $("#am_id_edit").change(function(){
        ...
        $('#checked_quantity_edit').val($remaining_lining_quantity);
        $('#remaining_lining_quantity_hidden_edit').val($remaining_lining_quantity);
    ...
    $("#checked_quantity_add").blur(function(){
        $remaining_lining_quantity_hidden = $('#remaining_lining_quantity_hidden').val();
        $checked_quantity_add = $('#checked_quantity_add').val();
        if($checked_quantity_add == 0 && $("#for_checked").prop("checked") == true){
            alert('Quantity can not be zero');
        }else if(parseInt($checked_quantity_add) > parseInt($remaining_lining_quantity_hidden)){
            alert('Can not greater than Issue quantity');
            $('#checked_quantity_add').val($remaining_lining_quantity_hidden);
        }
    });
    
    $("#form_lining_list_edit").validate({
        rules: {
            e_id: {
                required: true
            },
    ...
    $('#form_lining_list_edit').ajaxForm({
        beforeSubmit: function () {
            return $("#form_lining_list_edit").valid(); // TRUE when form is valid, FALSE will cancel submit
        },
    ...
    $("#form_add_lining_list_details").validate({
        rules: {
            co_id: {
                required: true,
            },
    ...
    $('#form_add_lining_list_details').ajaxForm({
        beforeSubmit: function () {
            return $("#form_add_lining_list_details").valid(); // TRUE when form is valid, FALSE will cancel submit
        },
        success: function (returnData) {
            console.log('RD => ' + returnData);
            obj = JSON.parse(returnData);
//          $('#form_add_lining_list_details')[0].reset(); 
            // $("#form_add_lining_list_details").validate().resetForm(); //reset validation
            ...
            //refresh table
            $('#lining_list_details_table').DataTable().ajax.reload();
    ...
    $("#form_edit_lining_list_details").validate({
        rules: {
    ...
    $('#form_edit_lining_list_details').ajaxForm({
        beforeSubmit: function () {
            return $("#form_edit_lining_list_details").valid(); // TRUE when form is valid, FALSE will cancel submit
        },
        success: function (returnData) {
            console.log('RD => ' + returnData);
            obj = JSON.parse(returnData);           
    ...
    //jobber4
    $("#lining_list_details_table").on('click', '.lining_detail_edit_btn', function() {
        $("#pod_edit_loader").removeClass('hidden');

        $lining_detail_id = $(this).attr('lining_detail_id');

        $.ajax({
            url: "<?= base_url('admin/ajax-fetch-lining-details-for-edit') ?>",
            method: "post",
            dataType: 'json',
            data: {'lining_detail_id': $lining_detail_id,},
            success: function(data){
                console.log(data);
                $lining_detail = data.lining_detail;
                //alert($lining_detail.lining_detail_id)
                ...
                $("#co_id_edit").html("<option>"+$lining_detail.co_no+"</option>").trigger('change');
                $("#am_id_edit").html("<option>"+$lining_detail.art_no+"</option>").trigger('change');
                $("#lc_id_text_edit").val($lining_detail.leather_color);
                $("#fc_id_text_edit").val($lining_detail.fitting_color);
                $("#checked_quantity_edit").val($lining_detail.checked_quantity);
                $("#checked_alter_edit").val($lining_detail.rejection_quantity);
                $("#remarks_for_other_quantity_edit").html("<option>"+$lining_detail.remarks_for_other_quantity+"</option>").trigger('change');
                $("#remarks_edit").val($lining_detail.remarks);
                ...
                $("#lining_detail_id_edit").val($lining_detail.lining_detail_id);
...
// delete area 
    ...
    $(document).on('click', '.delete', function(){
        ...
        if(confirm("Are You Sure?")){
            ...
            $.ajax({
                url: "<?= base_url('admin/del-lining-details-list') ?>",
                dataType: 'json',
                type: 'POST',
