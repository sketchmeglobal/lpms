                    ...
                    <div class="row table-responsive">
                        <table class="table table-bordered table-hover table2excel">
                            ...
                            <?php } ?>
                                ...
                                <td style="text-align: right; width: 24mm;"><?= number_format($ppl->rate_foreign + $ppl->additional_charges, 2) ?></td>
                            ...
                            <tr>
                                <td colspan="5" style="text-align:left"><h6 class="mar_0"> <?= (!empty(trim($ppl->hand_machine)) && ($print_packing_list[0]->print_hand_ratio == 1)) ? 'Hand & Machine Ratio:' . $ppl->hand_machine . ', ' : '' ?><?= $ppl->leather_type_info . ', H.S.Code:' . $ppl->remark . ', ' . $ppl->metal_fitting . ', ' . $ppl->size . ', ' ?><?= ($ppl->brand != '') ? $ppl->brand : 'Unbranded' ?></h6></td>
                            </tr>
                        <?php
                            $last_iter = $iter;
...
// RoDTEP DECLARATION_DESCRIPTION IS HUGE AND CAN'T FIT HERE SO IT NEEDS SPECIAL ATTENTION
                        ...
                        ?>
                                <tr>
                            <td colspan="2" style="font-weight: bold; font-size: 12px;text-align:left; text-transform: uppercase"><h5 style="font-size: 11px;" class=""><?= $print_packing_list[0]->currency ?> <?= convertNumberToWord(round($tot_amnt - round($tot_amnt * ($print_packing_list[0]->discount / 100), 2), 2)) ?></h5></td>
                        ...
                        <tr>
                            <td colspan="2"></td>
                            <td colspan="2"><h6 class="" style="text-align:left;">(-)Less: Discount @ <?= $print_packing_list[0]->discount ?>%</h6></td>
                            <td colspan="1"><h5 class="" style="text-align: right"><b><?= round(($tot_amnt * ($print_packing_list[0]->discount / 100)), 2) ?></b></h5></td>
                        </tr>
                        <tr>
                            <td colspan="2"></td>
                            <td colspan="2"><h6 class="" style="text-align:left;">(+)Handling Charges</h6></td>
