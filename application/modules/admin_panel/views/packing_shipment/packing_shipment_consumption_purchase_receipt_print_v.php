                                ...
                                foreach ($result as $curr_key=>$con) {
                                    ...
                                    foreach($result as $key=>$val) {
                                        if ($val->item_name == $con->item_name) {
                                            ...
                                            ?>
                                <?php  $purchase_receipt_row = $this->db->group_by('purchase_order_receive_id')->order_by('purchase_order_receive_id', 'desc')->limit(2)->get_where('purchase_order_receive_detail', array('id_id' => $con->item_dtl))->result();  ?>
                                            <?php 
                                           if(count($purchase_receipt_row) > 0) {
                                               foreach($purchase_receipt_row as $p_r_r) {
                                                        $get_item_dtl_row = $this->db->get_where('item_dtl', array('item_dtl', 'id_id' => $p_r_r->id_id))->row()->c_id;
                                                        if($con->ig_id == 1 || $con->ig_id == 2 || $con->ig_id == 4) {
                                                        $color_id = $con->lc_id;
                                                        } else {
                                                        $color_id = $con->fc_id;
                                                        } 
                                                        if($get_item_dtl_row != $color_id) {
                                                            continue 2;
                                                        }
                                               }
                                            ?>
                                            <tr>
                                                ...
                                                <td nowrap>
                                                    <?php  
                                                        ...
                                                        echo $this->db->get_where('purchase_order_receive', array('purchase_order_receive_id' => $p_r_r->purchase_order_receive_id))->row()->purchase_order_receive_bill_no." [".
                                                        date('d-m-Y', strtotime($this->db->get_where('purchase_order_receive', array('purchase_order_receive_id' => $p_r_r->purchase_order_receive_id))->row()->purchase_order_receive_date))."] <b>- ".
                                                        $this->db->select('SUM(item_quantity) as item_quantity')->get_where('purchase_order_receive_detail', array('id_id' => $p_r_r->id_id, 'purchase_order_receive_id' => $p_r_r->purchase_order_receive_id))->row()->item_quantity."</b><br/>"; 
                                                        }
                                                    ?>
                                                </td>
                                            </tr>
