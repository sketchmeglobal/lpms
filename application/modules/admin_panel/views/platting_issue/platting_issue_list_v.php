...
<html lang="en">
<head>
    <title>Platting Issue List | <?=WEBSITE_NAME;?></title>
...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <!-- page head start-->
        <div class="page-head">
            <h3 class="m-b-less">Platting Issue</h3>
            <div class="state-information">
                <ol class="breadcrumb m-b-less bg-less">
                    <li><a href="<?=base_url('admin/dashboard');?>">Home</a></li>
                    <li class="active"> Platting Issue</li>
                </ol>
            </div>
        </div>
        <!-- page head end-->
                        ...
                        <div class="panel-body">
                            <?php 
                                if($view_permission != 'block'){
                                    ?>
                            <a href="<?= base_url('admin/platting-issue-add') ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add Platting Issue</a>
                                <?php
                            } 
                            ?>
...
<script>
    $(document).ready(function() {
        $('#reveive_purchase_order_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('admin/ajax-platting-issue-table-data')?>",
                "type": "POST",
                "dataType": "json",
            },
            //will get these values from JSON 'data' variable
            "columns": [
                { "data": "platting_issue_number" },
                { "data": "platting_issue_date" },
                { "data": "action" },
            ],
            //column initialisation properties
            "columnDefs": [{
                "targets": [2], //disable 'Image','Actions' column sorting
                "orderable": false,
            }]
        } );
    } );
    ...
    $(document).on('click', '.delete', function(){
        if(confirm('Are you sure?')){
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data:{tab: $tab, tab_pk: $pk_name, data_pk: $pk_value, ref_tab: $ref_tab},
